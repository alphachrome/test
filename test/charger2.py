...
class SB:
    ...
    def readU16(self, little_endian=True):
        """Read an unsigned 16-bit value from the specified register, with the
        specified endianness (default little endian, or least significant byte
        first)."""
        bat._idle()
        bat._transaction_start()
        bat._i2c_start()
        #bat._i2c_write_bytes([bat._address_byte(False), self.addr])
        bat._i2c_write_bytes([bat._address_byte(False)])
        bat._i2c_write_bytes([self.addr])
        #bat._i2c_stop()
        bat._i2c_idle()
        bat._i2c_start()
        bat._i2c_write_bytes([bat._address_byte(True)])
        bat._i2c_read_bytes(2)
        bat._i2c_stop()
        response = bat._transaction_end()
...
MODE_INTERNAL_CHARGE_CONTROLLER = (1 << 0)
MODE_PRIMARY_BATTERY_SUPPORT    = (1 << 1)
MODE_CONDITION_CYCLE            = (1 << 7)
MODE_CHARGE_CONTROLLER_ENABLED  = (1 << 8)
MODE_PRIMARY_BATTERY            = (1 << 9)
MODE_ALARM                      = (1 << 13)
MODE_CHARGER                    = (1 << 14)
MODE_CAPACITY                   = (1 << 15)
...
SB_MODE     = SB("MODE", 0x03)
SB_STATUS   = SB("STATUS", 0x16)
...
SB_LIST = [ SB_TEMP, SB_MODE, SB_STATUS, SB_V, SB_I, SB_V_CHG, SB_I_CHG, SB_I_AVG, SB_REL_SOC, SB_ABS_SOC, SB_REM_CAP, SB_FULL_CAP,
            SB_FULL_CAP, SB_T_RUN2EMPTY, SB_T_AVG2FULL, SB_R, SB_R_TFULL, SB_R_TEMPTY,  SB_R_OK,
...
def print_bat():
    for reg in SB_LIST:
        val = reg.readU16()
        print (">> {} = {} {}".format(reg.name, val, reg.unit))
...
def cmd_test():
    set_bits("V_SYS_MAX", 7800)
    set_bits("I_ADP_LIM1", 3200)
...
cmd_list = [
    Cmd('b', 'Battery status', print_bat), 
    ...
    Cmd('test', 'Set V_SYS_MAX to 7800, I_ADP_LIM1 to 3200 for efficiency test', cmd_test)
]
